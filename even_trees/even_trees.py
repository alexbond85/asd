...
class SimpleTree:
    ...
    def _is_even_and_greater_zero(self, n: int):
        if n == 0:
            return False
        return n % 2 == 0

    def EvenTrees(self):
        nodes_total = self.Count()
        if nodes_total == 0:
            return []
        if nodes_total % 2 == 1:
            return []
        ...
        while len(starting_nodes) > 0:
            for n in starting_nodes:
                for ch in n.Children:
                    ch_total_nodes = len(self._get_all_nodes_subtree(ch))
                    if self._is_even_and_greater_zero(ch_total_nodes):
                        edges += [n, ch]
                    next_starting_nodes.append(ch)

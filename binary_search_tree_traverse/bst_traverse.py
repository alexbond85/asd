...
class BST:
    ...
    def _in_order(self, root: BSTNode):
        if root is None:
            ...
            left_nodes = self._in_order(root=root.LeftChild)
            right_nodes = self._in_order(root=root.RightChild)
            res = list(left_nodes) + [root] + list(right_nodes)
            return tuple(res)
    ...
    def _post_order(self, root: BSTNode):
        if root is None:
            ...
            left_nodes = self._post_order(root=root.LeftChild)
            right_nodes = self._post_order(root=root.RightChild)
            res = list(left_nodes) + list(right_nodes) + [root]
            return tuple(res)
    ...
    def _pre_order(self, root: BSTNode):
        if root is None:
            ...
            left_nodes = self._pre_order(root=root.LeftChild)
            right_nodes = self._pre_order(root=root.RightChild)
            res = [root] + list(left_nodes) + list(right_nodes)
            return tuple(res)
    ...
    def WideAllNodes(self):
        if self.Root is None:
            ...
            nodes = [self.Root]
            nodes_children = self._children(self.Root)
